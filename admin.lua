...
local function ch_addgroup2(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupadd2") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.addUserGroup(id,"Police Officer")
        vRPclient.notify(player,{"You add a police officer "..id})
      end)
    end
end

local function ch_removegroup2(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupremove2") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.removeUserGroup(id,"Police Officer")
		vRP.addUserGroup(id,"Unemployed")
        vRPclient.notify(player,{"You have resigned a policeman "..id})
      end)
   end
end

local function ch_addgroup3(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupadd3") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.addUserGroup(id,"EMS Paramedic")
        vRPclient.notify(player,{"You add a paramedic "..id})
      end)
    end
end  

local function ch_removegroup3(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupremove3") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.removeUserGroup(id,"EMS Paramedic")
		vRP.addUserGroup(id,"Unemployed")
        vRPclient.notify(player,{"You have resigned a paramedic "..id})
      end)
   end
end

local function ch_addgroup4(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupadd4") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.addUserGroup(id,"Member Sicilia")
        vRPclient.notify(player,{"You add a Member Sicilia "..id})
      end)
    end
end  

local function ch_removegroup4(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupremove4") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.removeUserGroup(id,"Member Sicilia")
		vRP.addUserGroup(id,"Unemployed")
        vRPclient.notify(player,{"You have resigned a sicilia "..id})
      end)
   end
end

local function ch_addgroup5(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupadd5") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.addUserGroup(id,"Member Narcos")
        vRPclient.notify(player,{"You add a Member Narcos "..id})
      end)
    end
end  

local function ch_removegroup5(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil and vRP.hasPermission(user_id,"player.groupremove5") then
    vRP.prompt(player,"User id: ","",function(player,id)
      id = parseInt(id)
        vRP.removeUserGroup(id,"Member Narcos")
		vRP.addUserGroup(id,"Unemployed")
        vRPclient.notify(player,{"You have resigned a narcos "..id})
      end)
   end
end
...
local function ch_revive(player,choice) 
    vRP.prompt(player,"Player ID:","",function(player,target_id) 
        if target_id ~= nil and target_id ~= "" then 
            nplayer = vRP.getUserSource(tonumber(target_id))
...
local function ch_givecoins(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
    vRP.prompt(player,"Amount:","",function(player,amount) 
      amount = parseInt(amount)
      vRP.giveCoins(user_id, amount)
    end)
  end
end
...
vRP.registerMenuBuilder("main", function(add, data)
  ...
  if user_id ~= nil then
    ...
    -- build admin menu
    choices["Lider Menu"] = {function(player,choice)
      vRP.buildMenu("lider", {player = player}, function(menu)
        menu.name = "Lider Menu"
  ...
  if user_id ~= nil then
    ...
    choices["Admin"] = {function(player,choice)
      vRP.buildMenu("admin", {player = player}, function(menu)
        ...
        if vRP.hasPermission(user_id,"revive.player") then
          menu["Revive"] = {ch_revive}
        end
